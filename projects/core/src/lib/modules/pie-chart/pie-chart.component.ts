...
@Component({
  ...
  template: '<ng-content select="bizy-pie-chart-section"></ng-content>',
  styles: [`
    :host {
      display: flex;
...
export class BizyPieChartComponent {
  ...
  readonly #pieChartService = inject(BizyPieChartService);

  @ContentChildren(BizyPieChartSectionComponent) sections: QueryList<BizyPieChartSectionComponent> | null = null;

  @Input() resizeRef: HTMLElement | null = null;
  @Input() centerLabel: string | null = null;
  @Input() tooltip: {show?: boolean, formatter?: (item: any ) => string} = this.#pieChartService.getTooltipConfig();
  @Input() legend: {show?: boolean, orient?: 'vertical' | 'horizontal', position?: {x: 'auto' | 'left' | 'right' | 'center', y: 'auto' | 'top' | 'bottom' | 'center'}} = this.#pieChartService.getLegendConfig();
  @Input() download: {show?: boolean, label?: string, fileName?: string} = this.#pieChartService.getDownloadConfig();
  @Input() expand: { show?: boolean, label?: string} = this.#pieChartService.getExpandConfig();
  @Input() label: { show?: boolean, overflow?: 'break' | 'truncate', line?: boolean, formatter?: (item: any ) => string} = this.#pieChartService.getLabelConfig();
